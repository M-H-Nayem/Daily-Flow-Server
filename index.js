...
async function run() {
  try {
    ...
    app.get("/tasks", async (req, res) => {
      ...
      let query = { user_email: email };
      try {
        const tasks = await tasksCollection.find(query).sort({deadline:1}).toArray();
        res.send(tasks);
      } catch (error) {
...
app.get("/exams", async (req, res) => {
      ...
      let query = { user_email: email };
      try {
        const tasks = await examsCollection.find(query).sort({examDate:1}).toArray();
        res.send(tasks);
      } catch (error) {
    ...
    app.put("/exams/:id", async (req, res) => {
      try {
        ...
        delete updatedExam._id;

        console.log('wil update',id,updatedExam);
        const filter = { _id: new ObjectId(id) };
        const updateExm = { $set: updatedExam };
        const result = await examsCollection.updateOne(filter, updateExm);
        if (result.matchedCount === 0) {
